...
let shader;
let buffer, bufferB;
let palette = ["#231942", "#5E548E", "#9F86C0", "#BE95C4", "#8A430A"];
let particles = [];
let colors = [];
let angle = 0.0;
...
function setup() {
  ...
  pixelDensity(1);
	...
	for (let i = 0; i < MAX_NUMBER; i++){
		let p = [random(-0.2, 0.2), random(-0.2, 0.2), random(0.03, 0.4)];
		particles.push(p);
...
function draw() {
	...
	for (let i  = 0; i < particles.length; i++){
		let sp = speed * (1.0 + 0.05 * particles[i][2]);
...
// Prepare data to send to the shader;
